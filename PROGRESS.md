...
**최종 갱신**: 커밋 13 `feat(a11y-keyboard)` 준비 완료 (미커밋)
...
커밋 13 `feat(a11y-keyboard)` 완료, **커밋 승인 대기 중**.

roving tabindex로 **Tab 4,000번 → 5번**. `↑↓` 카드 / `←→` 단계 이동 / `Enter` 상세 / `Delete` 불합격.
컬럼별 마지막 포커스 카드를 기억해 되돌아올 때 맨 위로 튕기지 않게 함.

**codex 교차 리뷰가 이번엔 성공했고, 내가 놓친 것을 잡았다** — 커밋 12에서 상세 패널의 포커스 복원은
고쳤는데 보드에서 방향키로 옮길 때는 같은 문제가 남아 있었다(포커스가 body로 떨어짐).
rAF로는 안 되고 목록이 다시 그려진 뒤(useEffect) 찾아야 했다.

다음: 커밋 14 `feat(virtualization)` — 1,000건 가상 스크롤
